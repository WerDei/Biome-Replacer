**What is this about?**  
Before 3.0, there was an oversight in how BR replaced biomes, which saved some extra data to your world. In Minecraft 26.1, this extra data will make the game believe your world is "corrupted".
...
## Firstly, update BR to (at least) 3.0
...
Download a .zip file from the following link, and extract it anywhere you like:
...
## Fix it
1. Drag the file from the previous step into the NBT Explorer window, navigate to the `dimensions` tag and open it (look at the picture below).
...
3. Select `biome_source` again and add a String tag with a "Aa" button, enter "preset" as the name and click OK. Double click it, and enter "minecraft:overworld" in it
4. Repeat steps 2 and 3 for the `minecraft:the_nether` tag, but enter `minecraft:nether` instead of `minecraft:overworld` in step 3. Yes, "nether" without "the" - this is important!
