...
public abstract class ModdedBiomeSlicesManagerMixin
{
    ...
    private static void biome_replacer$storeDimensionHash(Args args, @Local(ordinal = 0) Identifier location)
    {
        if (location != null)
    ...
    private static void biome_replacer$storeDimensionHashExtended(Args args, @Local(ordinal = 0) Identifier location)
    {
        if (location != null)
            BlueprintReplacer.pushDimensionContext(location.toString());
    }
